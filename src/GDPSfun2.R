...
prop <- function(state, t.new, Delta.t, newcbf, por, statei = NULL, Rf = 1, sorb = FALSE){
  ...
  # note that v = 1 if not vdepD; -phi is used because the co-ordinates are on the LHS of %*% (%*% is not commutative)
  # if 3D dispersion, then the rotation is applied about a vertical axis, as if the velocity is purely horizontal, which is truer to the way MODFLOW respresents flow within a layer
