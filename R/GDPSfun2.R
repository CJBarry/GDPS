...
# function to calculate the mass distribution from one particle after one
#  timestep
# returns a matrix with six columns: x, y, z, L, z_off, m
prop <- function(state, t.new, Delta.t, newcbf, por, statei = NULL, Rf = 1,
                 sorb = FALSE, curr.mfts){
  t.old <- t.new - Delta.t
  ...
  write(gd.PTR(state[, list(x = x - MFxy0[1L],
                            ...
                            L, zo)], t.old - MFt0),
        paste0(dmrt, ".ptr"))
  # only rewrite when more particles are required (saves memory otherwise)
  if(np > MXP.def) write(dattxt(por, np, dis), paste0(dmrt, ".dat"))
  ...
  ptldf[, t := t + MFt0]
  #
  # - adjust the timestep number to account for the fact that only a subset
  #    of the data arrays are loaded
  ptldf[, timestep := timestep - curr.mfts[1L] + 1L]
  #
  # - run MODPATHmass
  mt <- ptl.masstrack(ptldf, gwdata, wtop, por, state$m,
                      retain.storage = TRUE,
  ...
  if(all(xyzm$m == 0)){
    xyzm <- xyzm[, llply(.SD, last), by = ptlno,
                 .SDcols = c("x", "y", "zo", "L", "m")]
    xyzm[, ptlno := NULL]
    setcolorder(xyzm, colord)
    return(xyzm) 
    # note will return if nrow(xyzm) == 0, which is good
    #  (all(logical(0L)) = TRUE)
  }
  ...
  # finds the positions relative to the original particle (without rotation
  #  yet - in the longitudinal and transverse dimensions) using a random
  #  normal scattering in a random orientation
  npd <- np*Ndisppairs
  relpos <- if(ThreeDD){
    # distance
  ...
  # MODFLOW timestep number (for subset of arrays)
  mfts <- cellref.loc(t.new, c(0, gwdata$time) + MFt0)
  if(is.na(mfts)){
    ...
    mfts <- cellref.loc(t.new - Delta.t/100, c(0, gwdata$time) + MFt0)
  }
  
  if(ThreeDD) xyzm[, c("zo", "L") := {
    C <- cellref.loc(x, gwdata$gccs + MFxy0[1L], FALSE)
    R <- cellref.loc(y, gwdata$grcs + MFxy0[2L], TRUE)
    bot <- gwdata$elev[cbind(C, R, L + 1L)]
    top <- wtop[cbind(C, R, L, mfts)]
    
    NLAY <- dim(gwdata$elev)[3L] - 1L
    nwet <- mapply(function(ct, rt) sum(!is.na(wtop[ct, rt,, mfts])), C, R)
    # depth divides
    # the water table is repeated for the number of dry or partially
    #  saturated cells in the column and then the layer bottoms are used
    # the length of ddivs will always be NLAY + 1
    # should ensure that a particle is never labelled with a layer that is
    #  dry at its location
    ddivs <- mapply(function(ct, rt){
      c(rep(wtop[ct, rt, NLAY - nwet + 1L, mfts], NLAY - nwet + 1L),
        gwdata$elev[ct, rt, seq(to = NLAY, by = 1L, length.out = nwet) + 1L])
    }, ct, rt)
...
# finds particles which are close together and lumps them to new particle
#  at centre of mass - essential moderater of particle count
# additionally coalesces particles below a certain mass with nearest non-
#  insignificant particle - this happens after the initial coalescence and
#  avoids spending time tracking particles with negligible mass whilst
#  still ensuring conservation of mass
# expected output columns: 1 - x, 2 - y, 3 - z, 4 - zo, 5 - L, 6 - m
#
coalesce <- function(state, cd, mm = 0, t){
  ...
  #
  # - column references
  C <- cellref.loc(state$x, gwdata$gccs + MFxy0[1L], F)
  #
  # - row references
  R <- cellref.loc(state$y, gwdata$grcs + MFxy0[2L], T)
  #
  # - time step number (within the array subset)
  mfts <- cellref.loc(t, c(0, gwdata$time) + MFt0, F)
  lthk <- state[, {
  ...
  #    dealt within the prop function and should not really be considered
  #    as an error as it is a naturally possible occurrence
  ...
  # work out from the lmzo values what layers and z offsets should be given
  #  to the particles
  #
